  ...
  return (
    <div className="p-8 pb-20 max-w-[1400px] mx-auto">
      <PageHeader
        ...
        action={
          <Link href="/staff/booking" className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-[var(--hc-primary)] hover:bg-[var(--hc-primary-hover)] text-white rounded-[var(--radius-md)] transition-all">
      ...
      <section className="mt-8 hc-kpi-grid">
        ...
        <KpiCard label="No Shows" value="02" helper={<span className="text-slate-400">SLA rate: 94.5%</span>} icon={AlertTriangle} tone="red" />
      ...
      <section className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 border border-[var(--hc-border-soft)] rounded-[var(--radius-xl)] shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">Launch wizard to register new patients and confirm appointments slotting.</p>
          </div>
        ...
        <div className="bg-white p-6 border border-[var(--hc-border-soft)] rounded-[var(--radius-xl)] shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">View active clinical queues, call patients, and manage room allocations.</p>
          </div>
        ...
        <div className="bg-white p-6 border border-[var(--hc-border-soft)] rounded-[var(--radius-xl)] shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">Need help? Open support incident tickets or call the system hotline.</p>
          </div>
                    ...
                    <td className="hc-td text-right">
                      {slot.status === "Available" ? (
                        <Link
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-[var(--hc-primary)] text-white rounded-[var(--radius-md)] hover:bg-[var(--hc-primary-hover)] transition-colors"
                        >
                          Book Slot
                        </Link>
                      ) : slot.status === "Booked" ? (
