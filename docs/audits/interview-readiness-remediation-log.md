...
| 9 | ✅ **DONE — P0-2 BOLA closed.** 13 PHI endpoints guarded via new `hasClinicalAccess` (NURSE arm scoped to CHECKED_IN/IN_PROGRESS) + `ClinicalAccessGuard`. Includes the `AppointmentController` bypass routes the frontend actually calls. CCCD endpoint returns 404-for-both to avoid an enumeration oracle; UUID-keyed endpoints 403-for-both. **PR #35, CI green incl. Backend Maven verify.** CI caught a `SELECT FOR SHARE` vs `readOnly` bug that passed locally — took 2 rounds. |
