# Hospital Management System Use Case Diagram

Status: aligned with the repository on 2026-04-26 after AI and internal assistant removal.

The source PlantUML diagram is in [HMS_UseCaseDiagram.puml](HMS_UseCaseDiagram.puml).

## Active Actors

- Guest
- Patient
- Doctor
- Nurse
- Accountant
- Admin
- Gmail API

## Active Use Case Groups

- Public content, doctor/department discovery, booking, and scoped public chatbot
- Patient portal claim, login, overview, appointments, lab results, messages, and profile
- Doctor appointment, medical record, lab result, follow-up, and prescription PDF workflows
- Nurse queue, check-in, and vital-sign workflows
- Accountant invoice, payment, pricing, and revenue workflows
- Admin users, departments, rooms, schedules, content, news, monitoring, audit logs, and inventory workflows

Removed assistant and external model use cases are intentionally absent.
