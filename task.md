...
| 4 | **P1-7 api-client** — per-scope Map + `.finally()`-on-promise + threaded `_retried` (per C19) | Isolated, already unit-tested, zero UI blast radius | S |
| 5 | **C18 harden `ui/dialog.tsx`** (focus trap/restore, Escape, ARIA) | Prerequisite for items 7-8; benefits 8 existing consumers | S |
| 6 | **P0-3 demo boot** — `.env.demo.example` (C7) + `.gitignore` `.env.demo` + README:326-327 + document doctor3/4 | Unblocks every demo; verify C11 first | M |
...
| 11 | **P1-9 metrics** (36→34, 73→58, 611+→**633 measured**) + P0-4 untrack `task.md`/`recent_changes.diff` | Docs-only | S |
