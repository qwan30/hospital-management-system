...
function compareVersionishDesc(left, right) {
  return right.localeCompare(left, undefined, { numeric: true, sensitivity: "base" });
}

function listPluginCacheSkillRoots() {
  const cacheRoot = path.join(os.homedir(), ".codex", "plugins", "cache", "khuym", "khuym");
  ...
  return fs
    ...
    .sort(compareVersionishDesc)
    .map((version) => path.join(cacheRoot, version, "skills"));
}

function resolveSkillsRoot(options, repoRoot) {
  if (options.skillsRoot) {
    return path.resolve(options.skillsRoot);
  }

  const candidates = [
    path.join(repoRoot, "plugins", "khuym", "skills"),
    path.join(repoRoot, ".codex", "plugins", "khuym", "skills"),
    ...listPluginCacheSkillRoots(),
  ];

  return candidates.find((candidate) => fs.existsSync(candidate)) || candidates[0];
}

function formatSkillFilePath(repoRoot, skillFile) {
  const relative = path.relative(repoRoot, skillFile);
  return relative.startsWith("..") ? skillFile : relative;
}
...
function defaultCommandProbe(command, envPath = process.env.PATH || "") {
  ...
  for (const segment of envPath.split(path.delimiter).filter(Boolean)) {
    for (const extension of extensions) {
      const candidate = path.join(segment, `${command}${extension.toLowerCase()}`);
      const upperCandidate = path.join(segment, `${command}${extension.toUpperCase()}`);
      const match = [candidate, upperCandidate].find((filePath) => fs.existsSync(filePath));
      if (match && canExecute(match)) {
        return {
          available: true,
          detail: match,
        };
...
export function collectKhuymSkillDependencies(options = {}) {
  ...
  const skillsRoot = resolveSkillsRoot(options, repoRoot);
  ...
  for (const filePath of files) {
    ...
    declarations.push({
      ...parsed,
      skill_file: formatSkillFilePath(repoRoot, parsed.skill_file),
    });
  }
